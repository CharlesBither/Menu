...
public class Inventories {
    ...
    public Inventory menuInventory(String uuid) {
        ...
        for (int i = 0; i < inventory.getSize(); i++) {
            ...
            else if (i == 14) { inventory.setItem(i, menuInventoryItems.scoreboard()); }
    ...
    public Inventory warpInventory() {
        Inventory inventory = Bukkit.createInventory(null, 36,ChatColor.GREEN + "Warps");
        //Creates and opens warp menu
        for (int i = 0; i < inventory.getSize(); i++) {
            if (i == 12) { inventory.setItem(i, warpInventoryItems.spawn()); }
            else if (i == 13) { inventory.setItem(i, warpInventoryItems.shop()); }
            else if (i == 14) { inventory.setItem(i, warpInventoryItems.crates()); }
            else if (i == 21) { inventory.setItem(i, warpInventoryItems.survival()); }
            else if (i == 22) { inventory.setItem(i, warpInventoryItems.resource()); }
            else if (i == 23) { inventory.setItem(i, warpInventoryItems.creative()); }
            else if (i == 35) { inventory.setItem(i, menuInventoryItems.star()); }
            else {
                inventory.setItem(i, new ItemStack(Material.GRAY_STAINED_GLASS_PANE));
            }
        }
    ...
    public Inventory mediaInventory() {
        Inventory inventory = Bukkit.createInventory(null, 18, "Social Media");
        for (int i = 0; i < inventory.getSize(); i++) {
            if (i == 3) { inventory.setItem(i, mediaInventoryItems.initializeDiscord()); }
            else if (i == 4) { inventory.setItem(i, mediaInventoryItems.initializeWebsite()); }
            else if (i == 5) { inventory.setItem(i, mediaInventoryItems.initializeIg()); }
            else if (i == 17) { inventory.setItem(i, menuInventoryItems.star()); }
            else {
                inventory.setItem(i, new ItemStack(Material.GRAY_STAINED_GLASS_PANE));
            }
        }
